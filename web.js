AFRAME.registerComponent("web", {
  ...
  init: function () {
    ...
    */
    ...
    this.model.setAttribute("color", "#272727");
  ...
  toggleWeb: function (hand, value) {
    ...
    if (this.state == "off" && value > 0) {
      ...
      if (raycaster.intersections.length) {
        ...
        this.shootingVelocityInPercent =
  ...
  updateWeb: function (hand, rigLocalPosition, timeDeltaSec, offsetCamera) {
    ...
    this.handLocalPosition =
      this.handLocalPosition == this.handLocalPosition1
        ? this.handLocalPosition2
        : this.handLocalPosition1; // silly, needed for line to update
    ...
    if (this.state == "shooting") {
      this.shootingCurrentPercent +=
        this.shootingVelocityInPercent * timeDeltaSec;
  ...
  drawWeb: function () {
    ...
    */
    ...
    obj.position.multiplyScalar(
      1 - 0.5 * this.shootingCurrentPercent * this.shootingCurrentPercent
    );
    obj.scale.z =
      this.handLocalPosition.length() *
      this.shootingCurrentPercent *
      this.shootingCurrentPercent;
    obj.quaternion.setFromUnitVectors(
      new THREE.Vector3(0, 0, 1),
      this.handLocalDirection
    );
...
AFRAME.components[
  "vr-mode-ui"
].Component.prototype.toggleEnterARButtonIfNeeded = function () {
